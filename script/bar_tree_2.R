...
algo <- dbcan %>% dplyr::select(-gene_id) %>% 
  group_by(filename) %>% count(cazyme)
...
for (i in 1:nrow(fileList_effe)){
  ...
  nc_effector <- lfile %>%
    ...
    #                         .default = NA)) %>% 
    mutate(type = case_when(type == "Number of predicted effectors" ~ "Effector", 
                            .default = NA)) %>% 
    drop_na()
...
cazyme_df <- cazyme %>% 
  ...
  group_by(name) %>% count(CAZy) %>% 
 # count(Familia) %>% # count family (type)
  ungroup() %>% 
  pivot_wider( names_from = CAZy, values_from = n) %>% 
  replace(is.na(.), 0) %>% 
...
# establish order by factor
cazymeEffectors$type<- factor(cazymeEffectors$type, levels = c("Effector", "CAZyme - Effector"))
...
effector_ggplot <- ggplot(cazymeEffectors, 
  ...
  scale_fill_manual(values = c("CAZyme - Effector" = "darkblue",
                               "Effector" = "grey50"), 
                    name = NULL) +
...
ggsave("output/2023-12-20_phylo_cazyme_effectors.pdf", g, width = 20, height = 10, units = "in", dpi = 600)

...
rtree <- read.tree("input/rooted_tree_newick_short.txt")
...
dire_effe <-list.files(path="input/effectorP_seq/", recursive=TRUE, full.names = T)
dire_effe <- grep(dire_effe, pattern='effectorP.out', invert=FALSE, value=TRUE)
file_effe <-  str_split(str_split(dire_effe, "/", simplify = T)[,3],
...
prot_dire <-list.files(path="input/dbcan_run/", recursive=TRUE, full.names = T)
...
prot_file <- str_replace(
  str_split(
    str_split(prot_dire, "/", simplify = T)[,3],
...
cazymeEffectors <- cazyme %>% 
  ...
  mutate(type = "CAZyme - Effector") %>% 
  rbind(effectorTotal)

# establish order by factor
cazymeEffectors$type<- factor(cazymeEffectors$type, levels = c("CAZyme - Effector", "Apoplastic", "Cytoplasmic"))
...
tree_order <- p3$data %>% 
  ...
  left_join(sptreename %>% 
            ...
            by =c("label" = "file_name"))


# . get proteome/secretome plot ----

# establish order by tree label order
prot_scre$filename <- factor(prot_scre$filename, level = tree_order$label)

prot_scre <- drop_na(prot_scre)
...
ps_ggplot <- ggplot(prot_scre, 
  ...
  scale_fill_manual(values = c("proteome" = "#5593FF", 
...
# establish order by tree label order
all_families$filename <- factor(all_families$filename, level= tree_order$label)

all_families<- drop_na(all_families)
...
cazyme_ggplot <- ggplot(all_families, 
  ...
  scale_fill_manual(values = c("GH" = "#5593FF", 
                               "GT" = "darkorange2", 
                               "CE" = "grey50",
                               "PL" = "darkgreen", 
                               "AA" = "darkred", 
...
# establish order by tree label order
cazymeEffectors$filename <- factor(cazymeEffectors$filename, level= tree_order$label)
cazymeEffectors<- drop_na(cazymeEffectors)
...
effector_ggplot <- ggplot(cazymeEffectors, 
  ...
  scale_fill_manual(values = c("CAZyme - Effector" = "#5593FF",
                               "Apoplastic" = "grey50", 
                               "Cytoplasmic" = "forestgreen"), 
                    name = NULL) +
...
ggsave("plots/phylo_cazyme_effectors.pdf", g, width = 20, height = 10, units = "in", dpi = 600)
